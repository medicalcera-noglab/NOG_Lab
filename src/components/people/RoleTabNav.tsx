...
export function RoleTabNav({ tabs, activeRole }: RoleTabNavProps) {
  return (
    <nav aria-label="Filter team by role">
      <ul
        role="list"
        className="scrollbar-hide border-border flex gap-2 overflow-x-auto border-b pb-px sm:flex-wrap"
      >
        {tabs.map((tab) => {
          ...
          return (
            <li key={tab.role}>
              <Link
                ...
                className={cn(
                  'inline-flex shrink-0 items-center gap-1.5 rounded-t-md px-4 py-2 text-sm font-medium whitespace-nowrap',
